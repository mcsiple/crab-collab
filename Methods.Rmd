...
library(here)
library(tidyverse)
crabdat <- read.csv(here('Heeia_crabdata_Apr2019.csv'))
...
$$
where $L$ is the length at which selectivity is being estimated, $L_{S50}$ is the length at 50% selectivity, and $L_{S95}$ is the length at 95% selectivity. Following Nadon (2017), we used the discontinuous break in size composition of kept male crabs as the size at 95% selectivity ($L_{S95}$= 169 mm). 
